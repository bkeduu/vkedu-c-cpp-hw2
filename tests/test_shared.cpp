...
TEST(test_getopt_bad_param, get_params_tests) {
    string_t argv[] = { (string_t)"./a.out", (string_t)"file1.txt", (string_t)"file2.txt", (string_t)"-j", (string_t)"1" };
    int argc = 5;
    size_t proc_count = 0;
    size_t files_count = 0;
    string_t* file_names = NULL;
...
TEST(test_handle_more_proc, handle_files_tests) {
...
TEST(test_handle_files_bad_files_one_proc, handle_files_tests) {
    string_t file_names[] = {(string_t)"file1.txt", (string_t)"file2.txt"};
    EXPECT_EQ(ERR_FOPEN, handle_files(file_names, 2, 1));
}

TEST(test_handle_files_bad_files_zero_proc, handle_files_tests) {
    string_t file_names[] = {(string_t)"file1.txt", (string_t)"file2.txt"};
    EXPECT_EQ(ERR_FOPEN, handle_files(file_names, 2, 0));
}
